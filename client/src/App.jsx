...
import logo from './assets/buddy.png'; // <-- IMPORT LOGO
...
function DashboardLayout() {
  ...
  // GLOBAL STATE: This array holds all successfully scheduled posts!
    ...
    <div className="flex h-screen w-full relative overflow-hidden font-sans text-[#1d1d1f] bg-[#f8f9fb]">
      ...
      <aside className="w-64 bg-white/80 backdrop-blur-2xl border-r border-white/80 hidden md:flex flex-col p-6 z-20 shadow-[10px_0_40px_-15px_rgba(0,0,0,0.05)]">
        ...
        <div className="flex-1">
          <h3 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-4">Pipeline</h3>
          <ul className="space-y-3">
            {scheduledPosts.length === 0 ? (
                <li className="text-sm font-medium text-gray-400 italic">No scheduled posts</li>
            ) : (
                scheduledPosts.map((post, idx) => (
                    <li key={idx} className="text-sm font-medium text-gray-600 flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-green-500"></div>
                        {post.title} ({post.dayNum})
                    </li>
                ))
            )}
          </ul>
      ...
      <div className="flex-1 flex flex-col relative h-full z-10">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-white/80 flex items-center justify-end px-8 z-20 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.03)]">
          <div className="flex items-center gap-6 text-gray-600">
            <button className="hover:text-black transition-colors relative">
                <Bell size={20} />
                {scheduledPosts.length > 0 && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full border-2 border-white"></span>}
            </button>
        ...
        <main className="flex-1 overflow-y-auto pb-32">
          {/* We pass the global state down to all pages using Outlet context */}
        ...
        <nav className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-white/70 backdrop-blur-3xl border border-white shadow-2xl rounded-full px-2 py-2 flex gap-2 z-50">
          {navItems.map((item) => {
            ...
            return (
              <button key={item.path} onClick={() => navigate(item.path)} className={`flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm transition-all duration-300 ${isActive ? 'bg-black text-white shadow-lg scale-105' : 'text-gray-500 hover:bg-black/5 hover:text-black'}`}>
